...
using BlockDiagonals: Diagonal
using LinearAlgebra

function single_frequency_tscs(x::Vector{Float64}, freq::Float64, c0::Float64, aa::Float64)
    ## number of cylinders
    M = Int(length(x) / 2)
...
x = DE.coords_to_x(env.config.pos)
single_frequency_tscs(x, 0.6, 1480.0, 1.0)